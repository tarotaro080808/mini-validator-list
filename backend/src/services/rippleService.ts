...
export default class RippleService implements IRippleService {
  ...
  private async _mergeAll() {
    return this._cacheManager.set(Cache.TYPES.MERGED_DATA, async () => {
      await Promise.all([
        this._cacheManager.waitFor(Cache.TYPES.RIPPLE_DEFAULT_UNL),
  ...
  async getValidatorInfo() {
    await this._cacheManager.waitFor(Cache.TYPES.MERGED_DATA);
